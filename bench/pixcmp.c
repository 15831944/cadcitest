...
main(int argc, char *argv[])
{
	...
	if (print_bytes) {
	    bu_log("\t%s: %7llu bytes (%8zu bytes, skipping %7zu)\n",
		   argv[0], (unsigned long long)(sf1.st_size - f1_skip), sf1.st_size, f1_skip);
	    bu_log("\t%s: %7llu bytes (%8zu bytes, skipping %7zu)\n",
		   argv[1], (unsigned long long)(sf2.st_size - f2_skip), sf2.st_size, f2_skip);
	} else {
	    bu_log("\t%s: %7llu pixels (%8zu bytes, skipping %7zu)\n",
		   argv[0], (unsigned long long)((sf1.st_size - f1_skip)/3), sf1.st_size, f1_skip);
	    bu_log("\t%s: %7llu pixels (%8zu bytes, skipping %7zu)\n",
		   argv[1], (unsigned long long)((sf2.st_size - f2_skip)/3), sf2.st_size, f2_skip);
	}
